...
# 生徒ごとの集計
students = (
    data.groupby("名前")["教科", "点数"].apply(lambda x: x.tolist()).reset_index()
)
...
def calculate_report(student):
    ...
    for subject in range(1, 6):
        scores = [score for (subj, score) in student[1] if subj == subject]
...
for idx, report in enumerate(reports):
    ...
    # 判定の決定
    scores = [score for (_, score) in student[1]]
